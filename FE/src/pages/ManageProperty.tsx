...
const INITIAL_PROPERTY = {
  ...
  features: ["Swimming Pool", "Garden", "Smart Home", "24/7 Security"],
  overview: "Một đoạn mô tả ngắn gọn và cuốn hút về điểm nổi bật nhất của ngôi nhà. Thường hiển thị đầu tiên tại tab Overview.",
  galleryObjects: [
    {
      url: "https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?auto=format&fit=crop&q=80&w=600",
      caption: "Trải nghiệm cuộc sống đẳng cấp tại biệt thự ven sông với thiết kế hiện đại, nội thất nhập khẩu và không gian mở thân thiện với thiên nhiên."
    }
  ],
  specs: [
...
const MOCK_REQUESTS = [
  { id: 101, username: "Le Hoang", avatar: "", date: addDays(new Date(), 2), time: "10:00 AM", status: "pending" },
  { id: 102, username: "Tran Thi B", avatar: "", date: addDays(new Date(), 3), time: "14:30 PM", status: "pending" },
  { id: 103, username: "Nguyen Van C", avatar: "", date: addDays(new Date(), 1), time: "09:00 AM", status: "confirmed" }
];
...
const ManageProperty = () => {
  ...
  const [feedbackInput, setFeedbackInput] = useState<number | null>(null);
  ...
  const updateRequestStatus = (reqId: number, status: string) => {
    setRequests(reqs => reqs.map(r => r.id === reqId ? { ...r, status } : r));
    setFeedbackInput(null); // Close feedback if open
  };

  const sendFeedback = (reqId: number) => {
    if (!feedbackText.trim()) return;
    // Mock sending feedback
    console.log(`Sending feedback for request ${reqId}: ${feedbackText}`);
    setFeedbackText("");
    setFeedbackInput(null);
    updateRequestStatus(reqId, 'feedback_sent');
  };
      ...
      <main className="pt-28 pb-16 max-w-[1440px] mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-[450px] flex-shrink-0 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-border p-6 sticky top-28">
                <div className="flex items-center justify-between mb-6">
                    <h3 className="text-lg font-bold font-['Inter']">Requests</h3>
                    <span className="bg-rose-100 text-rose-700 font-bold px-2.5 py-0.5 rounded-full text-sm">
                ...
                <div className="flex flex-col gap-4 max-h-[600px] overflow-y-auto pr-2 -mr-2">
                    {requests.length === 0 ? (
                        <div className="text-center py-8 text-slate-500 italic">No appointment requests yet.</div>
                    ) : (
                        requests.map(req => (
                            <div key={req.id} className={cn(
                                "border rounded-xl p-4 transition-all relative overflow-hidden",
                                req.status === 'pending' ? "border-sky-200 bg-sky-50/30 shadow-sm" : "border-slate-100 bg-slate-50"
                            )}>
                                {/* Status Indicator line */}
                                {req.status === 'confirmed' && <div className="absolute left-0 top-0 bottom-0 w-1 bg-emerald-500"></div>}
                                {req.status === 'denied' && <div className="absolute left-0 top-0 bottom-0 w-1 bg-rose-500"></div>}
                                {req.status === 'feedback_sent' && <div className="absolute left-0 top-0 bottom-0 w-1 bg-amber-500"></div>}

                                <div className="flex items-start gap-3 mb-4">
                                    <Avatar className="w-10 h-10 border border-slate-200">
                                        <AvatarImage src={req.avatar} />
                                        <AvatarFallback className="bg-slate-200 text-slate-700 font-bold"><User className="w-5 h-5"/></AvatarFallback>
                                    </Avatar>
                                    <div className="flex-1">
                                        <div className="font-bold text-slate-900">{req.username}</div>
                                        <div className="text-sm font-semibold text-[#0F766E] flex items-center gap-1.5 mt-0.5">
                                            <CalendarIcon className="w-3.5 h-3.5" />
                                            {format(req.date, "dd MMM")} at {req.time}
                                        </div>
                                    </div>
                                    ...
                                    {req.status !== 'pending' && (
                                        <div className={cn(
                                            "text-xs font-bold px-2 py-1 rounded capitalize border",
                                            req.status === 'confirmed' && "bg-emerald-100 text-emerald-800 border-emerald-200",
                                            req.status === 'denied' && "bg-rose-100 text-rose-800 border-rose-200",
                                            req.status === 'feedback_sent' && "bg-amber-100 text-amber-800 border-amber-200"
                                        )}>
                                ...
                                {req.status === 'pending' && (
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <Button 
                                            size="sm" variant="outline"
                                            onClick={() => setFeedbackInput(feedbackInput === req.id ? null : req.id)}
                                            className="w-full text-slate-600 border-slate-200"
                                        >
                                            <MessageSquare className="w-4 h-4 mr-1.5" /> Propose Different Time / Feedback
                                        </Button>
                                    </div>
                                )}

                                {/* Feedback Form */}
                                {feedbackInput === req.id && (
                                    <div className="mt-3 bg-white p-3 rounded-lg border border-slate-200 animate-in fade-in zoom-in-95">
                                        <textarea 
                                            placeholder="Gợi ý một khung giờ khác mà bạn rảnh..."
                                            className="w-full text-sm p-2 border border-slate-200 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 mb-2"
                                            rows={2}
                                            value={feedbackText}
                                            onChange={e => setFeedbackText(e.target.value)}
                                        />
                                        <div className="flex justify-end gap-2">
                                            <Button size="sm" variant="ghost" onClick={() => setFeedbackInput(null)} className="h-7 px-2 text-xs">Cancel</Button>
                                            <Button size="sm" onClick={() => sendFeedback(req.id)} className="h-7 px-3 text-xs bg-blue-600 hover:bg-blue-700">Send</Button>
                                        </div>
                                    </div>
                                )}
                            </div>
        ...
        <div className="mt-8 flex justify-between items-center border-t border-slate-200 pt-8">
            <Button 
                variant="outline"
                className="border-rose-200 text-rose-600 hover:bg-rose-50 hover:text-rose-700 font-bold px-6 py-6 text-base rounded-xl"
                onClick={() => {
            ...
            <div className="flex items-center gap-4">
                {saveSuccess && <span className="text-emerald-600 font-semibold animate-fade-in flex items-center gap-1"><CheckCircle2 className="w-5 h-5"/> Saved successfully</span>}
                <Button 
                    onClick={handleSave} 
                    disabled={isSaving}
                    className="bg-[#0F766E] hover:bg-[#0F766E]/90 text-white shadow-md font-bold px-10 py-6 text-lg rounded-xl"
                >
                    {isSaving ? "Saving..." : <><Save className="w-5 h-5 mr-3" /> Save Changes</>}
                </Button>
            </div>
        </div>
