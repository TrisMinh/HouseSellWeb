...
// Dummy stats (sáº½ Ä‘Æ°á»£c thay báº±ng API riÃªng sau)
...
const BUY_LIST = Array.from({ length: 15 }, (_, i) => ({
  id: i + 1, 
  property: i % 2 === 0 ? `Garden Villa â€“ Ecopark ${i + 1}` : `Premium Shophouse ${i + 1}`,
...
const Profile = () => {
  ...
  // Real appointments/sell listings state
  ...
  const displayBuyList = appointments.length > 0 ? 
    appointments.map(a => ({
      ...
      price: "â€”", // Backend DTO doesn't include price yet, but that's fine for now
  ...
  const profileStats = {
    totalValue: sellListings.length
      ? `${new Intl.NumberFormat("vi-VN").format(totalSellValue)} VND`
      : "—",
  ...
  // Láº¥y giÃ¡ trá»‹ tá»« user thá»±c, fallback vá» empty string
  ...
  const displayName = user ? `${user.first_name} ${user.last_name}`.trim() || user.username : 'â€”';
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="bg-white rounded-2xl pt-3 px-4 pb-6 border border-gray-100 shadow-sm relative">
                  ...
                  <div className="absolute top-3 right-3">
                    {!editingInfo ? (
                      <button onClick={() => setEditingInfo(true)} className="p-2 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer" title="Sá»­a">
                        <Edit className="w-4 h-4 text-gray-400" />
                      </button>
                    ) : (
                      <div className="flex gap-1">
                        <button onClick={() => { setEditingInfo(false); }} className="p-2 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer" title="Há»§y">
                          <X className="w-4 h-4 text-gray-400" />
                        </button>
                        <button onClick={handleSaveInfo} disabled={savingInfo} className="p-2 rounded-lg bg-[#0F766E] hover:bg-[#0F766E]/90 transition-colors cursor-pointer disabled:opacity-60" title="LÆ°u">
                      ...
                      <div key={info.label} className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          {editingInfo && info.editable ? (
                            ...
                            <div className="text-sm text-gray-700 font-medium">{infoData[info.key] || <span className="text-gray-400 italic">ChÆ°a cáº­p nháº­t</span>}</div>
                          )}
                        </div>
                      </div>
                    ))}
                ...
                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm relative">
                  ...
                  <div className="absolute top-3 right-3">
                    {!editingBio ? (
                      <button onClick={() => setEditingBio(true)} className="p-2 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer" title="Sá»­a">
                        <Edit className="w-4 h-4 text-gray-400" />
                      </button>
                    ) : (
                      <div className="flex gap-1">
                        <button onClick={() => setEditingBio(false)} className="p-2 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer" title="Há»§y">
                          <X className="w-4 h-4 text-gray-400" />
                        </button>
                        <button onClick={handleSaveBio} disabled={savingBio} className="p-2 rounded-lg bg-[#0F766E] hover:bg-[#0F766E]/90 transition-colors cursor-pointer disabled:opacity-60" title="LÆ°u">
                  ...
                  {editingBio ? (
                    ...
                    <p className="text-sm text-gray-500 leading-relaxed mb-5">{bioData.bio || <span className="italic text-gray-400">ChÆ°a cÃ³ giá»›i thiá»‡u.</span>}</p>
                  )}
                  <div className="space-y-3 border-t border-gray-100 pt-4">
                    {[
                      { label: "Há»", editKey: "firstName" as const, display: bioData.firstName, editable: true },
                      { label: "TÃªn", editKey: "lastName" as const, display: bioData.lastName, editable: true },
                      ...
                      <div key={d.label} className="flex items-center text-sm">
                        ...
                        {editingBio && d.editable && d.editKey ? (
                          ...
                          <span className="font-medium text-gray-700">{d.display || <span className="italic text-gray-400">â€”</span>}</span>
                        )}
                      </div>
                    ))}
                  </div>
                      ...
                      options={{
                        ...
                        plugins: {
                          ...
                          tooltip: {
                            ...
                            cornerRadius: 10,
                            callbacks: {
                              label: (ctx) => ` Revenue: ${ctx.parsed.y} Tá»· VNÄ`,
                            },
                          },
                        },
                        scales: {
                ...
                <div className="bg-white rounded-2xl pt-3 px-4 pb-6 border border-gray-100 shadow-sm relative">
                  <div className="flex justify-between items-center mb-5">
                    ...
                    <button
                      ...
                      title={activityVisible ? "Äang hiá»‡n cho ngÆ°á»i khÃ¡c" : "Äang áº©n vá»›i ngÆ°á»i khÃ¡c"}
                    >
                      {activityVisible ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
                      {activityVisible ? "CÃ´ng khai" : "ÄÃ£ áº©n"}
                    </button>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      { label: "Urgent", title: "Meeting with buyer for Biá»‡t thá»± ven sÃ´ng", time: "27 Feb, 2:30 PM â€“ 4:00 PM", color: "bg-red-500" },
          ...
          {activeTab === "buy" && (
            <div className="space-y-6 animate-[fadeIn_0.3s_ease]">
              ...
              <div className="grid grid-cols-1 gap-4">
                {loadingAppointments ? (
                    <div className="py-10 text-center text-gray-500">Äang táº£i dá»¯ liá»‡u...</div>
                ) : (
